export const STARS_NUM = 4000;

export const DISTANCE_LIMIT = 3000;

export const starTypes = {
  percentage: [
    0.04, 0.03, 0.01, 0.04, 0.06, 0.06, 0.02, 0.01, 0.04, 0.06, 0.1, 0.1, 0.46,
  ],
  color: [
    0xffcece, 0xffe8ce, 0xceffe6, 0xcef9ff, 0xd2ceff, 0xff9d9d, 0xfffa9d,
    0xb9ff9d, 0x9db9ff, 0xca9dff, 0x6445ff, 0x4570ff, 0xffffff,
  ],
  size: [1, 0.5, 0.1, 0.8, 0.3, 0.5, 0.1, 0.1, 1.3, 1.3, 1.1, 1.1, 0.5],
};

export const ARMS_X_DIST = 5000;
export const ARMS_Z_DIST = 1000;
export const GALAXY_THICKNESS = 300;
export const SPIRAL = 1.2;
export const SPIRAL_START = 1000;
export const STARS_DENSITY = 0.4;

export const SPACE_MIN_SIZE = -50000;
export const SPACE_MAX_SIZE = 50000;
export const DIMENSION = 3;

export const BACKGROUND_STARS_NUM = 500;
export const BACKGROUND_STAR_COLORS = [
  "#8fa8f6",
  "#b4ffb8",
  "#ffdd8f",
  "#ff8fba",
];
export const BACKGROUND_STAR_SIZE = 1.5;
